...
while True:
    ...
    for i, e in call.items():
        while e[0] != '0':
            ...
            if ratioActual > ratioEntrada * (1 + 0.1) and saldo > 10 and strikeCall(e[0]).precio_BI() != 1000 != strikeCall(e[3]).precio_OF():
                ...
                if strikeCall(e[0]).cantidad_BI() >= e[1] and strikeCall(e[0]).cantidad_OF() >= e[4]:
                    ...
                    print('Ratio positivo , pero no hay suficientes compradores/vendedores ',time.strftime("%H:%M:%S")),time.sleep(2)
                    break
            else:
